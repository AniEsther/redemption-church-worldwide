...
router.post('/', requireAdmin, async (req, res, next) => {
  try {
    ...
    if (!videoUrl?.trim() && !audioUrl?.trim() && !downloadUrl?.trim()) {
      return res.status(400).json({ error: 'Provide at least one of Video, Audio, or Download.' })
    }
...
router.put('/:id', requireAdmin, async (req, res, next) => {
  try {
    const { videoUrl, audioUrl, downloadUrl } = req.body
    if (!videoUrl?.trim() && !audioUrl?.trim() && !downloadUrl?.trim()) {
      return res.status(400).json({ error: 'Provide at least one of Video, Audio, or Download.' })
    }
